...
def create_team_router(team_service: TeamService) -> APIRouter:
    ...
    async def post_team_submission(
        team_id: uuid.UUID,
        ...
        try:
            ...
            logger.error("team_submission_failed", team_id=str(team_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
            raise HTTPException(status_code=500, detail="Failed to submit team")
    ...
    async def get_event_teams(
        ...
        try:
            ...
            logger.info(
                "event_teams_fetched", event_id=str(event_id), teams_count=len(teams)
            )
            ...
            logger.error(
                "event_teams_fetch_failed", event_id=str(event_id), error=str(e)
            )
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
            raise HTTPException(status_code=500, detail="Failed to fetch event teams")
    ...
    async def get_user_teams(
        ...
        try:
            ...
            logger.info(
                "user_teams_fetched", user_id=str(user_id), teams_count=len(teams)
            )
            return GetTeamsResponse(
                teams=[GetTeamResponse.from_model(t) for t in teams]
            )
            ...
            logger.error("user_teams_fetch_failed", user_id=str(user_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
    ...
    async def get_user_event_team(
        ...
        try:
            ...
            if team is None:
                logger.warning(
                    "user_event_team_not_found",
                    user_id=str(user_id),
                    event_id=str(event_id),
                )
                raise HTTPException(status_code=404, detail="Team not found")
            logger.info(
                "user_event_team_fetched",
                user_id=str(user_id),
                event_id=str(event_id),
                team_id=str(team.id),
            )
            ...
            logger.warning(
                "event_or_user_not_found", user_id=str(user_id), event_id=str(event_id)
            )
            raise HTTPException(status_code=404, detail="Event or user not found")
        except Exception as e:
            logger.error(
                "user_event_team_fetch_failed",
                user_id=str(user_id),
                event_id=str(event_id),
                error=str(e),
            )
    ...
    async def post_teams(
        ...
        try:
            ...
            logger.warning(
                "owner_not_found", team_id=str(team_id), owner_id=str(request.owner_id)
            )
            raise HTTPException(status_code=404, detail="Owner not found")
        except Exception as e:
            logger.error("team_creation_failed", team_id=str(team_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
    ...
    async def put_teams(
        ...
        try:
            ...
            logger.error("team_update_failed", team_id=str(team_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
    ...
    async def delete_team_member(
        ...
        logger.info(
            "team_member_removal_started", team_id=str(team_id), user_id=str(user_id)
        )

        try:
            await team_service.kick_member(team_id, user_id)
            logger.info(
                "team_member_removed", team_id=str(team_id), user_id=str(user_id)
            )
        except TeamNotFoundError:
            logger.warning("team_not_found", team_id=str(team_id))
            raise HTTPException(status_code=404, detail="Team not found")
    ...
    async def put_member_role(
        ...
        span.set_attribute("role.id", str(request.role_id))

        logger.info(
            "member_role_change_started",
            team_id=str(team_id),
        ...
        try:
            await team_service.change_member_role(team_id, member_id, request.role_id)
            logger.info(
                "member_role_changed",
                team_id=str(team_id),
                member_id=str(member_id),
            ...
            logger.warning("member_not_found", member_id=str(member_id))
