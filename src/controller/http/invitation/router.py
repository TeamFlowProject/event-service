...
def create_invitation_router(service: InvitationService) -> APIRouter:
    ...
    async def create_invitation(request: CreateInvitationRequest):
        ...
        try:
            ...
            logger.warning("invitation_already_exists",
                           invitation_id=str(invitation_id))
            raise HTTPException(
                status_code=409, detail="Invitation already exists")
        except ParticipantAlreadyInTeam:
            logger.warning("participant_already_in_team",
                           invitation_id=str(invitation_id))
            raise HTTPException(
                status_code=409, detail="Participant already in a team")
        except Exception as e:
            logger.error("invitation_creation_failed",
                         invitation_id=str(invitation_id), error=str(e))
    ...
    async def get_invitation(invitation_id: uuid.UUID):
        ...
        logger.info("invitation_fetch_started",
                    invitation_id=str(invitation_id))
        ...
        try:
            ...
            logger.warning("invitation_not_found",
                           invitation_id=str(invitation_id))
            raise HTTPException(status_code=404, detail="Invitation not found")
        except Exception as e:
            logger.error("invitation_fetch_failed",
                         invitation_id=str(invitation_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
            raise HTTPException(
                status_code=500, detail="Failed to fetch invitation")
    ...
    async def get_invitations_by_team(team_id: uuid.UUID):
        ...
        try:
            ...
            logger.info("team_invitations_fetched", team_id=str(
                team_id), count=len(invitations))
            ...
            logger.error("team_invitations_fetch_failed",
    ...
    async def delete_invitation(invitation_id: uuid.UUID):
        ...
        logger.info("invitation_deletion_started",
                    invitation_id=str(invitation_id))
        ...
        try:
            ...
            logger.error("invitation_deletion_failed",
                         invitation_id=str(invitation_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
            raise HTTPException(
                status_code=500, detail="Failed to delete invitation")
    ...
    async def decide_invitation(invitation_id: uuid.UUID, request: DecisionRequest):
        ...
        try:
            ...
            logger.info("invitation_decision_completed", invitation_id=str(
                invitation_id), decision=request.decision.value)
        except InvitationNotFoundError:
            logger.warning("invitation_not_found",
                           invitation_id=str(invitation_id))
            raise HTTPException(status_code=404, detail="Invitation not found")
            ...
            logger.error("invitation_decision_failed",
                         invitation_id=str(invitation_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
            raise HTTPException(
                status_code=500, detail="Failed to decide invitation")
    ...
    async def create_join_request(request: CreateJoinRequestRequest):
        ...
        try:
            ...
            logger.info("join_request_created",
                        join_request_id=str(created_id))
            return {"id": str(created_id)}
        except TeamNotFoundError:
            logger.warning("team_not_found", team_id=str(request.team_id))
            raise HTTPException(status_code=404, detail="Team not found")
            ...
            logger.warning("role_not_found",
                           join_request_id=str(join_request_id))
            raise HTTPException(status_code=404, detail="Role not found")
        except JoinRequestAlreadyExistsError:
            logger.warning("join_request_already_exists",
                           join_request_id=str(join_request_id))
            raise HTTPException(
                status_code=409, detail="JoinRequest already exists")
        except ParticipantAlreadyInTeam:
            logger.warning("participant_already_in_team",
                           join_request_id=str(join_request_id))
            raise HTTPException(
            ...
            logger.error("join_request_creation_failed",
                         join_request_id=str(join_request_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
            raise HTTPException(
                status_code=500, detail="Failed to create join request")
    ...
    async def get_my_join_requests(
        ...
        try:
            ...
            logger.info("join_requests_fetched", track_id=str(
                track_id), user_id=str(x_user_id), count=len(join_requests))
            return GetJoinRequestsResponse(
                join_requests=[_join_request_to_response(
                    jr) for jr in join_requests]
            )
        except Exception as e:
            logger.error("join_requests_fetch_failed", track_id=str(
                track_id), user_id=str(x_user_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
            raise HTTPException(
                status_code=500, detail="Failed to fetch join requests")
    ...
    async def get_join_request(track_id: uuid.UUID, join_request_id: uuid.UUID):
        ...
        try:
            ...
            logger.info("join_request_fetched",
                        join_request_id=str(join_request_id))
            return _join_request_to_response(join_request)
        except JoinRequestNotFoundError:
            logger.warning("join_request_not_found",
                           join_request_id=str(join_request_id))
            raise HTTPException(
                status_code=404, detail="JoinRequest not found")
        except Exception as e:
            logger.error("join_request_fetch_failed",
                         join_request_id=str(join_request_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
            raise HTTPException(
                status_code=500, detail="Failed to fetch join request")
    ...
    async def cancel_join_request(
        ...
        try:
            ...
            logger.info("join_request_cancelled",
                        join_request_id=str(join_request_id))
        except JoinRequestNotFoundError:
            logger.warning("join_request_not_found",
                           join_request_id=str(join_request_id))
            ...
            logger.error("join_request_cancellation_failed",
                         join_request_id=str(join_request_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
            raise HTTPException(
                status_code=500, detail="Failed to cancel join request")
    ...
    async def decide_join_request(join_request_id: uuid.UUID, request: DecisionRequest):
        ...
        try:
            ...
            logger.info("join_request_decision_completed", join_request_id=str(
                join_request_id), decision=request.decision.value)
        except JoinRequestNotFoundError:
            logger.warning("join_request_not_found",
                           join_request_id=str(join_request_id))
            raise HTTPException(
            ...
            logger.error("join_request_decision_failed",
                         join_request_id=str(join_request_id), error=str(e))
            span.record_exception(e)
            span.set_status(trace.StatusCode.ERROR, str(e))
            raise HTTPException(
                status_code=500, detail="Failed to decide join request")
