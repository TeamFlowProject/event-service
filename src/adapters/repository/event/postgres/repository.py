...
from src.adapters.repository.event.postgres.queries import CREATE_EVENT_QUERY, UPDATE_EVENT_QUERY, DELETE_EVENT_QUERY, \
    UPDATE_PARTICIPANT_QUERY, SELECT_EVENT_QUERY, CREATE_PARTICIPANT_QUERY, SELECT_PARTICIPANT_QUERY, \
    REGISTER_PARTICIPANT_QUERY, SELECT_EVENTS_QUERY_BY_ID, SELECT_EVENTS_QUERY_BY_NUM, SELECT_PARTICIPANTS_QUERY_BY_NUM, \
...
class EventPostgresRepository:
    ...
    async def get_event_by_id(self, event_id: uuid.UUID) -> Event | None:
        ...
        async with self._pool.connection() as conn:
            async with conn.transaction():
                async with conn.cursor(row_factory=psycopg.rows.class_row(EventRow)) as cursor:
                    await cursor.execute(
                        SELECT_EVENT_QUERY,
                        {"id": str(event_id)}
                    )
    ...
    async def get_events_page_by_id(self, event_id: uuid.UUID, limit: int) -> list[Event]:
        """
    ...
    async def get_participants_by_id(self, event_id: uuid.UUID, participant_id: uuid.UUID, limit: int) -> list[Participant]:
        ...
        async with self._pool.connection() as conn:
            async with conn.transaction():
                async with conn.cursor(row_factory=psycopg.rows.class_row(ParticipantRow)) as cursor:
                    await cursor.execute(
                        SELECT_PARTICIPANTS_QUERY_BY_ID,
                        {"event_id": str(event_id), "participant_id": str(participant_id), "limit": limit}
                    )
                    rows = await cursor.fetchall()
    ...
    async def get_participants_by_num(self, event_id: uuid.UUID, offset: int, limit: int) -> list[Participant]:
        """
